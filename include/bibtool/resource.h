...
RSC_NEXT('c')
  ...
  RscByFct(   "check.rule"	      , r_cr  ,add_check_rule(SymbolValue(val)))
...
RSC_NEXT('e')
  ...
  RscByFct(   "extract.file"	      , r_ef  ,read_aux(SymbolValue(val),save_input_file,false))
...
RSC_NEXT('i')
  RscByFct(   "input"		      , r_i   ,save_input_file((char*)SymbolValue(val))  )
...
RSC_NEXT('m')
  RscByFct(   "macro.file"	      , r_mf  ,save_macro_file((char*)SymbolValue(val)))
...
RSC_NEXT('o')
  RscByFct(   "output.file"	      , r_of  ,save_output_file((char*)SymbolValue(val)) )
