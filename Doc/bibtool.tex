%%*****************************************************************************
%% $Id: bibtool.tex,v 1.5 2007-02-08 19:47:16 gene Exp $
%%=============================================================================
%%
...
\newcommand\sh@@[1]{\texttt{bibtool }\textit{#1}
        \smallskip\par\noindent\ignorespaces}
...
picture on \BibTool{} is shown. The next chapter afer this one is then devoted
...
\begin{description}
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/distribs/}{\BibTeX}] is
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibclean/}{bibclean}]
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/biblook/}{bibindex/biblook}]
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibsort/}{bibsort}]
  is a UNIX shell script by Nelson H.F.~Beebe to sort a \BibTeX{} file.
  
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibextract/}{bibextract}]
  is a UNIX shell script by Nelson H.F.~Beebe to extract entries from a
  \BibTeX{} file which are used in a \LaTeX{} document.
    
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/lookbibtex/}{lookbibtex/bibdestringify}]
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibtools/}{bibtools}]
...
\item [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibview/}{bibview}] is a Perl script by Dana Jacobsen to extract entries from a
  \BibTeX{} file which are used in a \LaTeX{} document.
    
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibcard/}{BibCard}]
  is a program by William C.~Ogden running under X11/xview which provides a
  means to edit bibliographic databases.
  
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/hyperbibtex/}{hyperbibtex}]
  Something similar for Macintosh computers.
    
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/xbibtex/}{xbibtex/bibprocess/bibsearch}]
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/bibview/}{bibview}]
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/tkbibtex/}{tkbibtex}]
...
\item [\Link{http://www.ctan.org/tex-archive/support/bibdb/}{bibdb}] Editor for \BibTeX{} files that runs under Dos and Windows.
  
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/qbibman/}{qbibman}]
  is a graphical user interface by Ralf G\"{o}rtz utilizing \BibTool{} as
  underlying library. It is written in C++ and uses Qt.
      
\item [\Link{http://barracuda.linuxbox.com/}{Barracuda}] an X11 Editor for
...
\item
  [\Link{http://www.ctan.org/tex-archive/biblio/bibtex/utils/btOOL/}{btOOL}]
...
neccesary to prevent the command line interpreter to gobble the special
characters.
...
\begingroup\footnotesize\label{sample1}%
\begin{verbatim}
...
This instruction selects all entries containing the substring \texttt{tex} in the
...
This example extracts all entries containing the substring \texttt{tex} in the
...
\begin{description}
\item[Note] Usually crossreferenced entries are not selected automatically.
  This can result in incomplete --- and thus incorrect --- \BibTeX\ files. To
  avoid this behaviour use the following command:
...
%------------------------------------------------------------------------------
\subsection{Correctly Sorting Crossreferenced Entries}

\BibTeX\ has a restriction that a crossreferenced entry has to come after the
...
This can be achieved with a resource file containing the following
...
\item [\texttt{\{\%1.\#s(crossref)a\#z\}}]\ \\
  If the first condition fails then the next alteranative after the hash mark
...
A tight integration of BibTool into another programming language is
possible. As an experiment into this direction I have choosen Tcl as the
target language. The result is BibTcl which is contained in the distribution
of \BibTool. 
...
\item \FTP{ftp.dante.de}{tex-archive/biblio/bibtex/utils/bibtool}  (Germany)
\item \FTP{ftp.tex.ac.uk}{tex-archive/biblio/bibtex/utils/bibtool} (UK)
\item \FTP{ctan.tug.org}{tex-archive/biblio/bibtex/utils/bibtool} (USA)
\end{list}
\BibTool{} can be found in the following directory:
\begin{list}{}{}
\item \texttt{tex-archive/biblio/bibtex/utils/bibtool}
\end{list}
...
report to my email address \texttt{gene@gerd-neugebauer.de}. Please
...
freely. See the file \Link{COPYING}{COPYING} for details.
...
\item Write a useful program and release it to the public without making
  profit, preferably under the\ Link{GPL.html}{GNU General Public License} or
  the GNU artistic license.
\end{itemize}
...
\item Boolean resources can take only the values \rsc{on} and \rsc{off}.
  The values \rsc{on}, \rsc{t}, \rsc{true}, 1, and \rsc{yes} are interpreted
  as the same. Everything else is interpreted as \rsc{off}.
...
this sample we also see the possibility to omit the equality sign and use
...
characters like \texttt{\symbol{"5C}}, \texttt{\$}, \texttt{\&},
\verb|!|, or \verb|#|.
...
resource is a string where each character represents an entry type to be
...
ascending (off) or descending (on) \ASCII{} order of the sort key.  The
...
\RSC{sort.format}{\textit{\%N(author) \# \%N(editor)}}
...
\RSC{preserve.key.case}{on}\vspace{-4ex}
\RSC{sort.cased}{on}
\smallskip
...
\item[The plus] (\verb|+|) is used to denote any number of repetitions of the
  ...
  E.g. the regular expression \emph{ab+c} matches any string which starts
  with a followed by one or more b and ended by a c. Thus it matches
...
\item[The question mark] (\verb|?|) is used to denote an optional regular
  ...
  E.g. the regular expression \emph{ab?c} matches any string which starts
  with a followed by at most one b and ended by a c. Thus it matches
...
\item[The bar] (\verb/\|/) separates two regular expressions. The combined
  ...
  Note that the bar has to be preceeded by a backslash.
  
  E.g. the regular expression \emph{abc\(\backslash\mid\)def} matches the
  string \emph{abc} and the string \emph{def}.
...
\item[Parentheses] (\verb|\(\)|) can be used to group regular expressions. A
  ...
  Note that the parantheses have to be preceeded by a backslash.
  
  E.g. the regular expression \emph{\(a\backslash(b\backslash\mid\backslash d)c\)}
  matches the strings \emph{abc} and \emph{adc}.
...
\item[The hat] (\texttt{\Hat}) matches the empty string at the beginning of
  ...
  E.g. the regular expression \emph{\Hat abc} matches the strings
...
\item[The brackets] (\verb|[]|) are used to denote a list of characters. If
  ...
  The regular expression \emph{[\Hat abc]} matches any single letter string not
  consisting of an a, b, or c.
  
\item[The backslash] (\texttt{\BS}) is used for several purposes. Primarily it can
...
\RSC{extract.file}{\textit{file.aux}}
...
Note that the extraction automatically respects the crossreferences in the
...
\RSC{select.by.string}{\textit{field\(_1\) \(\ldots\) field\(_n\) "string"}}
...
\RSC{select.by.non.string}{\textit{field\(_1\) \(\ldots\) field\(_n\) "string"}}
\begin{description}
\item[Note] Crossreferences are not considered unless \rsc{select.crossrefs}
  is set.
\end{description}
...
\RSC{select}{\textit{field\(_1\) \(\ldots\) field\(_n\) "regular\_expression"}}
...
field named \emph{field} which has a substring matching
...
\RSC{select.non}{\textit{field\(_1\) \(\ldots\) field\(_n\) "regular\_expression"}}
...
\RSC{extract.regex}{\textit{regular\_expression}}
...
\subsection{Extracting and Crossreferences}\label{sec:xref}

When extracting entries due to contained substrings or regular expression
matching crossreferenes are not considered automatically. This behaviour can
result in incomplete and thus incorrect \BibTeX\ files.

The automatic selection of crossreferenced entries can be controlled by the
resource \rsc{select.crossrefs}. This resource is \texttt|off| by default.
This means that crossreferences are ignored.

The following instruction can be used to turn on the automatic inclusion of
crossreferenced entries:

\Rsc{select.crossrefs}{on}

The depth of crossreference chains can be restricted with the resource
\rsc{crossref.limit}.
...
\begin{description}
  ...
  \item [\rsc{fmt.word.separator}]
        ...
        considered as word constituents. Word separators are whitespace and
        punctation characters. Those can not be redefined.
...
The \texttt{\%} character is followed by a single character --- here \verb|N|
...
\begin{itemize}
  \item [\texttt{p}] Format names according to the format specifier number
  ...
  \item [\texttt{n}] Format last names only.\\
        ...
        are more names they are treated as given as \texttt{and others}.
        ...
        This is the same as using the \texttt{p} format specifier with
        the post value of 0. The \textit{post} value of the \texttt{n}
  ...
  \item [\texttt{N}] Format names with last names and initials.\\
        In a list of names at most \textit{pre} last names are used. If there
        are more names they are treated as given as \texttt{and others}.
        If \textit{post} is greater than 0 then at most \textit{post}
        characters per name are used. Otherwise the whole name is
        ...
        This is the same as using the \texttt{p} format specifier with
        the post value of 1. The \textit{post} value of the \texttt{n}
        specifier is used as the \textit{len} value of the first item of
        the name format specifier. (See also section~\ref{sec:names})
  ...
  \item [\texttt{d}] Format a number, e.g.\ a year.\\
    ...
    \textit{pre} digits --- counted from the right --- are used. E.g.\ the
    field \texttt{"june 1958"} formatted with \texttt{\%2d} results in
    \texttt{58}.
    
    \textit{pre} defaults to a large number except in when the negative sign
    is present. Then it defaults to 1.
        
    \textit{post} defaults to 1. Thus if you want to select the second numer
  ...
  \item [\texttt{D}] Format a number.\\
        This format specifier acts like the \texttt{d} specifier except
  ...
  \item [\texttt{s}] Take a field as is (after translation of special
        characters).\\ 
  ...
  \item [\texttt{T}] Format sentences. Certain words are ignored.\\
        ...
        The string to be formatted according to this specification is
        separated into words. To accomplish this whitespace characters and
        punctation characters are considered to be not part of a word but as
        separator. To add additional word separators use the resource
        \rsc{fmt.word.separator}. In the following example the characters
        \verb|+|, \verb|-|, \verb|<|, \verb|=|, \verb|>|, \verb|*|, and
        \verb|/| are declared as additional word separators.
  ...
  \item [\texttt{t}] Format sentences. In contrast to the format letter
        \texttt{T} no words are ignored.\\
        At most \textit{pre} words are used. The other words are ignored. If
        \textit{pre} is 0 then no artificial limit is forced. If \textit{post}
        is positive then at most \textit{post} letters of each word are
        considered. Otherwise the complete words are used.
  ...
  \item [\texttt{\#w}] Count the number of words. All words are considered
    ...
    \(l>a\) or \(a>h\).
        ...
        \begin{Example}
          ...
          \begin{tabular}{ll}
            ...
            \texttt{\%-1,1\#w(title)}	& \textit{fails}
          \end{tabular}
        \end{Example}

   \item [\texttt{\#t}] Is the same as \texttt{\#w}.

   \item [\texttt{\#W}] Count the number of words. Certain words are
...
If some words are enclosed in brace, they are considered as one composed 
word. For example, with the format \verb|%t(title)|, and this field:
\begin{itemize}
  \item [] {\verb|title = "{The Whole Title}"|}
\end{itemize}
We obtain \verb|The-Whole-Title|.
...
behaviour of \BibTeX{} the special field \texttt{crossref} is taken into
...
\item [\texttt{@}\textit{type}]
  ...
  In an article item the specification \texttt{\%s(@Article)} succeeds and
  returns \texttt{Article} whereas \texttt{\%s(@Book)} fails.
...
The \texttt{\%}\verb|0s| specifier is used to check for the existence of a
certain field without actually adding anything to the output. Other constructs
may serve for the same purpose. This construct is applied to the pseudo-fields
\texttt{@book} and \texttt{@proceedings}. The access to the pseudo-field fails
if requested in another type of entry. Those two checks are combined to form a
disjunction.  Thus the following code---the constant in this example---is
reached only if we are in a book or in a proceedings entry. It is not reached
in an article.
...
\RSC{tex.define}{\textit{macro=replacement text}}
...
\RSC{tex.define}{\textit{macro}\texttt{[}\textit{arg}\texttt{]=}\textit{replacement text}}
...
braces would be ignored when formating field, otherwise the braces would 
remain.

\RSC{tex.define}{\BS protect[1]=\#1}
...
of the letters \texttt{f}, \texttt{v}, \texttt{l}, or \texttt{j}. Finally
there are three optional arguments enclosed in brackets.

Thus the general form looks as follows:

\begin{itemize}
  \item [] \texttt{\%}\textit{sign len letter}
           \texttt{[}\textit{mid}\texttt{][}%
           \textit{pre}\texttt{][}\textit{post}\texttt{]} 
\end{itemize}

The letter \texttt{f} denotes all first names.  The letter \texttt{l} denotes
all last names.  The letter \texttt{v} denotes all words in the von part.  The
letter \texttt{j} denotes all words in the junior part.
...
\begin{enumerate}
\item If a field named \texttt{bibkey} is present then the value of this
...
\item If the type of the entry is a volume of conference proceedings
  (\texttt{proceedings}) then the editor, the first 5 initials of the
...
\RSC{add.field}{\textit{field=value}}
...
\RSC{add.field}{time="June 13, 2000"}
...
\RSc{rewrite.rule}{\textit{\(\{\) field\(_1\) \(\ldots\) field\(_n\)
    \# pattern \#
    replacement\_text\(\}\)}} 
...
\RSc{rewrite.rule}{\(\{\) \textit{pattern \# replacement\_text}\(\}\)}
...
\RSc{rewrite.rule}{\(\{\)\textit{field \# pattern} \(\}\)}\\[-7ex]
\RSc{rewrite.rule}{\(\{\)\textit{pattern}\(\}\)}

More concrete, the rewrite rule

\RSc{rewrite.rule}{\(\{\) time \# "\Hat\(\{\}\)\$" \(\}\)}
...
\begin{itemize}
  \item Empty entries are composed of delimiters --- either double quotes or
        ...
        \RSc{rewrite.rule}{%
          \texttt{\symbol{"7B} "\symbol{"5E}\symbol{"5C}"\symbol{32}*\symbol{"5C}"\$" \symbol{"7D}}}\\[-9ex]
        \RSc{rewrite.rule}{%
          \texttt{\symbol{"7B} "\symbol{"5E}\symbol{"7B}\symbol{32}*\symbol{"7D}\$" \symbol{"7D}}}
        
        The caret '\texttt{\symbol{"5E}}' denotes the beginning of the whole
        string and the dollar is its end. The star is an operator which says
        that an arbitrary number of the preceding regular expression --- i.e.\ 
        the space --- can occur at this point.
  ...
  \item Ranges of pages should usually be composed of numbers separated by an
        ...
        \RSc{rewrite.rule}{\texttt{\symbol{"7B} pages \#
            "\symbol{"5C}(\symbol{"5B}0-9\symbol{"5D}+\symbol{"5C})\symbol{32}*-\symbol{32}*\symbol{"5C}(\symbol{"5B}0-9\symbol{"5D}+\symbol{"5C})" 
            = "\symbol{"5C} 1--\symbol{"5C} 2" \symbol{"7D}}}

  \item Field rewriting may be used to remove \LaTeX commands. This example 
        shows how to remove from titles a {\tt\char"5C protect} macro 
        together with the braces, in case the delimiter is a double quote.

        \RSc{rewrite.rule}{\tt\char"7B title \#
            "\char"5E\char"5C"\char32*\char"5C\char"5C
             \char32*protect\char32*\char"7B
             \char"5C(.*\char"5C)\char"7D\char"5C"\$"
            = "\char"5C"\char"5C 1\char"5C"" \char"7D}
...
\RSc{sort.order}{\(\{\) \textit{entry = field\(_1\) \# field\(_2\) \# ...} \(\}\)}
...
Another possibility is to specify the entry \texttt{*}. Such a sorting order is
...
sign \texttt{@} since this would undo the effect of deleteing an entry.
...
\RSc{check.rule}{ \(\{\) \textit{field \# pattern \# message}\(\}\)}
...
\RSc{check.rule}{\texttt{\symbol{"7B} year
 "\Hat[\BS"\symbol{"7B}]1[89][0-9][0-9][\BS"\symbol{"7D}]\$" \symbol{"7D}}}\\[-7ex]
\RSc{check.rule}{\texttt{\symbol{"7B} year "\Hat[\BS"\symbol{"7B}][0-9][0-9][\BS"\symbol{"7D}]\$" \symbol{"7D}}}\\[-7ex] 
\RSc{check.rule}{\texttt{\symbol{"7B} year "" "\BS@ \BS\$: Year has to be a suitable number"\symbol{"7D}}} 
...
error message. \texttt{\BS@} is replaced by the current type and \texttt{\BS\$} by
the current key.
...
\RSC{macro.file}{\textit{macro/file/name}}
...
\end{verbatim}
The macro \texttt{WGA} has been expanded and merged with \verb|1967|. Note that
...
\BibTeX{} file is translated to \verb|Ae|.\footnote{Note that the short
