%%*****************************************************************************
%% $Id: bibtool.tex,v 1.3 2007-02-08 05:35:57 gene Exp $
%%=============================================================================
%%
...
\item \texttt{ftp.dante.de}  (Germany)
\item \texttt{ftp.tex.ac.uk} (UK)
\item \texttt{ftp.cdrom.com} (US mirror for \texttt{shsu.edu})
\end{list}
...
Otherwise the default resource file (usually the file \texttt{.bibtoolrsc}) is
...
component is an optional equality sign (\texttt{=}). The equality sign
...
given then the extension \texttt{.bib} is appended and another read is tried. In
...
The same effect can be obtained by assigning the value \on\ or \off\ to the
resource \rsc{quiet}:
...
\begin{description}
  ...
  \item [\rsc{print.comma.at.end}]
    	...
    	should be printed at the end of the line. If it is \off{} then the
  ...
  \item [\rsc{print.equal.right}]
        ...
        field name. This value defaults to \on{}.
  ...
  \item [\rsc{print.use.tab}]
        ...
        spaces are inserted instead. This value defaults to \on{}.
...
ascending (\off) or descending (\on) \textsc{ascii} order of the sort key.
The sort key is initialized from the reference key if not given otherwise.
...
\textbf{Note} that in \textsc{ascii} order the case is important. The
...
feature can be used in addition to the selection according to \texttt{aux} files.
...
regular expression \texttt{"."} is used.
...
\begin{itemize}
  ...
  \item [\tt t] Format sentences. In contrast to the format letter \texttt{T} no
        words are ignored.\\
        At most \textit{pre} words are used. The other words are ignored. If
        \textit{pre} is 0 then no artificial limit is forced. If \textit{post}
        is positive then at most \textit{post} letters of each word are
  ...
  \item [\tt \#p] Count the number of names.
    ...
    \texttt{\%\textit{l}.\textit{h}\#p} succeeds if and only if $l\leq a\leq h$.\\
    \texttt{\%-\textit{l}.\textit{h}\#p} succeeds if and only if $l>a$ or $a>h$.
  ...
  \item [\tt \#n] Is the same as \texttt{\#p}.
  ...
  \item [\tt \#w] Count the number of words. All words are considered
   ...
   \item [\tt \#t] Is the same as \texttt{\#w}.
...
behaviour of \BibTeX{} the special field \texttt{crossref} is taken into account.
If a field is missing them the entry named in the \texttt{crossref} field is
...
resource \rsc{crossref.limit}. The number of uses of the \texttt{crossref} field
...
Suppose an \BibTeX{} entry contains fields for \texttt{editor} and \texttt{year}.
...
\texttt{year} field contains \verb|"1992"| then the result is \verb|itor:92|.
...
the same purpose. This construct is applied to the pseudo-fields \texttt{@book}
and \texttt{@proceedings}. The access to the pseudo-field fails if requested in
...
of the letters \texttt{f}, \texttt{v}, \texttt{l}, or \texttt{j}. Finally there are three
...
The letter \texttt{f} denotes all first names.  The letter \texttt{l} denotes all
last names.  The letter \texttt{v} denotes all words in the von part.  The letter
...
switch statement, or even better as a \texttt{cond} statement in Lisp. This means
...
Suppose a time stamp is stored in the field \texttt{time}.  With these resources
...
\emph{entry} is the name of an entry like \texttt{book}. The \emph{field}s are an
arbitrary number of field names like \texttt{author}. This specification says
...
\BibTeX{} file is translated to \verb|Ae|.\footnote{Note that the notation of
  \texttt{german.sty} is not understood by \BibTeX{} nor by \BibTool. }

Additionally some logos are defined.
\medskip\def\bs{\texttt{\char"5C}\ignorespaces}
