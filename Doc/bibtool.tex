%%*****************************************************************************
%% $Id: bibtool.tex,v 1.1 2007-02-07 21:28:27 gene Exp $
%%=============================================================================
%%
...
\newcommand\Desc[3]{{\it #1}&{\it #2}&#3\\\hline}
...
example:\footnote{Shamelessly stolen from the \BibTeX{} {\tt xamples.bib}
  file.}  Suppose it is contained in a file named {\sf sample.bib}.
...
those fields in the {\em select}\/ instruction:
...
In addition, this page page contains a description of the current version of
...
{\em installer}\/ if something is missing.
...
reading phase with your End-Of-File character (e.g.\ Control-C on UN*X, or
Control-Z on MS-D*S)
...
The following invocation of \BibTool{} uses only the resource file {\it
  my\_rsc}:

\sh{{\tt -r} my\_rsc {\tt -i} sample}

If you want to initialize the resources from the default resource file before
you can use the \opt{R} {\em before}\/ the inclusion of the resource file:
...
\begin{Summary}
  ...
  \Desc{}{\rsc{print} \{message\}}{Write out the text {\it message}.} 
  \Desc{\opt{r} file}{\rsc{resource} = file}{Immediately evaluate the
    instructions from the resource file {\it file}.} 
  \Desc{}{\rsc{resource.search.path}}{List of directories to search
    for resource files.} 
  \Desc{\opt{-} rsc}{rsc}{Evaluate the resource instruction {\it rsc}.}
\end{Summary}
...
\begin{Summary}
  \Desc{}{\rsc{bibtex.env.name}=\{var\}}{Use the environment variable {\it
      env}\/ to add more directories to the search path for \BibTeX{}
    (input) files.}
  \Desc{}{\rsc{bibtex.search.path}=\{path\}}{Use the list of directories
    {\it path}\/ to find \BibTeX{} (input) files.} 
  \Desc{}{\rsc{dir.file.separator}=\{c\}}{Use the character {\it c}\/ to
    separate the directory from the file.} 
  \Desc{}{\rsc{env.separator}=\{c\}}{Use the character {\it c}\/ to separate
    directories in a path.} 
  \Desc{\opt{i} file}{\rsc{input.file}\{file\}}{Add the \BibTeX{}
    file {\it file}\/ to the list of input files.}
\end{Summary}
...
this holds for sorting which does not recognized differences in case.

\begin{Summary}
  \Desc{}{\rsc{new.entry.type}\{type\}}{Define a new entry type {\it type}.}
  \Desc{}{\rsc{new.field.type}\{type\}}{Define a new field type {\it type}.}
  ...
  \Desc{}{\rsc{print.indent}=n}{Indent normal entries to column {\it n}.}
  \Desc{}{\rsc{print.line.length}=n}{Break lines at column {\it n}.}
  ...
  \Desc{}{\rsc{symbol.type}=type}{Translate symbols according to {\it
      type}\/: upper, lower, or cased.} 
\end{Summary}
...
{\bf Note} that in {\sc ascii} order the case is important. Usually the
...
\begin{Summary}
  ...
  \Desc{}{\rsc{sort.format}\{spec\}}{Add disjunctive branch {\it
      spec}\/ to the sort key specifier.}
...
\item[The period] (\verb|.|) matches any single character.
  ...
  E.g. the regular expression {\em a.c}\/ matches the string {\em abc}\/ but
...
\item[The star] (\verb|*|) is used to denote any number of repetitions of the
  ...
  E.g. the regular expression {\em ab*c}\/ matches any string which starts
  with a followed by an arbitrary number of b and ended by a c. Thus it
  matches {\em ac}\/ and {\em abbbc}. But it does not match the string {\em
    abcc}.
...
\item[The plus] (\verb|+|) is used to denote any number of repetitions of the
  ...
  with a followed by one or more b and ended by a c. Thus it matches {\em
    abbbc}.  But it does not match the string {\em ac}.
...
\item[The question mark] (\verb|?|) is used to denote an optional regular
  ...
  with a followed by at most one b and ended by a c. Thus it matches {\em
    abc}. But it does not match the string {\em abbc}.
...
\item[The bar] (\verb/|/) separates two regular expressions. The combined
  ...
  E.g. the regular expression {\em abc$\mid$def}\/ matches the string
  {\em abc} and the string {\em def}.
...
\item[Parentheses] (\verb/()/) can be used to group regular expressions. A
  ...
  E.g. the regular expression {\em a(b$\mid$d)c}\/ matches the strings
  {\em abc}\/ and {\em adc}.
...
\item[The dollar] (\verb|$|)%$
  ...
  E.g. the regular expression {\em abc\$}\/ matches the string {\em aaaabc}\/
...
\begin{description}
  \item [\rsc{preserve.keys}]
        This boolean resource determines whether exiting keys should
  ...
  \item [\rsc{preserve.key.case}]
        ...
        used exactly as read opposed to normalizing them by translating all
...
{\it pre}\/ and {\it post}\/ are positive integers whose meaning depends on
the format letter {\it letter}. {\it qualifier letter} is a one letter
...
\newenvironment{Example}{\smallskip\par{\it Example}\par}{\smallskip\par}
\begin{itemize}
  ...
  \item [\tt d] Format a number, e.g.\ a year.\\
        ...
        {\it pre}\/ defaults to a large number except in when the
        ...
        \begin{Example}
          ...
          \begin{tabular}{ll}
            ...
            \%.3d(pages)	& {\it fails}	\\
            \%+.3d(pages)	& 0		\\
            \%0d(pages)		& {\it succeeds with empty result}	\\
          \end{tabular}}
        \end{Example}
  ...
  \item [\tt s] Take a field as is (after translation of special
        ...
        \begin{Example}
          ...
          \begin{tabular}{ll}
            ...
            \%-8s(author)	& a.-u.-th	\\
            \%+8s(author)	& A.-U.-TH	\\
            \%0s(author)	& {\it succeeds with empty result}	\\
          \end{tabular}}
        \end{Example}
  ...
  \item [\tt \#p] Count the number of names.
    ...
    {\tt \%{\it l}.{\it h}\#p} succeeds if and only if $l\leq a\leq h$.\\
    {\tt \%-{\it l}.{\it h}\#p} succeeds if and only if $l>a$ or $a>h$.
    ...
    {\it pre}\/ and {\it post}\/ both defaults to 0.
        ...
        \begin{Example}
          ...
          {\tt
          \begin{tabular}{ll}
            \%2\#p(author)	& {\it succeeds with empty result}	\\
            \%4\#p(author)	& {\it fails}	\\
            \%-4\#p(author)	& {\it succeeds with empty result}	\\
            \%3.4\#p(author)	& {\it succeeds with empty result}	\\
            \%-3.4\#p(author)	& {\it fails}	\\
          \end{tabular}}
        \end{Example}
  ...
  \item [\tt \#s] Count the number of allowed characters.
    ...
    {\tt \%-{\it l}.{\it h}\#p} succeeds if and only if $l>a$ or $a>h$.
        ...
        \begin{Example}
          ...
          {\tt
          \begin{tabular}{ll}
            \%\#s(title)	& {\it succeeds with empty result}	\\
            \%13,13\#s(title)	& {\it succeeds with empty result}	\\
            \%10,16\#s(title)	& {\it succeeds with empty result}	\\
            \%-10,16\#s(title)	& {\it fails}	\\
          \end{tabular}}
        \end{Example}
  ...
  \item [\tt \#w] Count the number of words. All words are considered
    ...
    {\tt \%-{\it l}.{\it h}\#p} succeeds if and only if $l>a$ or $a>h$.
        ...
        \begin{Example}
          ...
          {\tt
          \begin{tabular}{ll}
            \%\#w(title)	& {\it succeeds with empty result}	\\
            \%3,3\#w(title)	& {\it succeeds with empty result}	\\
            \%1,6\#w(title)	& {\it succeeds with empty result}	\\
            \%-1,1\#w(title)	& {\it fails}	\\
          \end{tabular}}
        \end{Example}
   ...
   \item [\tt \#W] Count the number of words. Certain words are
    ...
    {\tt \%-{\it l}.{\it h}\#p} succeeds if and only if $l>a$ or $a>h$.
        ...
        \begin{Example}
          ...
          \begin{tabular}{ll}
            \%\#W(title)	& {\it succeeds with empty result}	\\
            \%2,2\#W(title)	& {\it succeeds with empty result}	\\
            \%1,6\#W(title)	& {\it succeeds with empty result}	\\
            \%-1,6\#W(title)	& {\it fails}	\\
          \end{tabular}}
        \end{Example}
...
succeeds then the {\it then-part}\/ is evaluated. Otherwise the {\it
  else-part}\/ is evaluated. Both parts may be empty. Nevertheless the braces
are required.
...
Certain format specifiers which act on lists of words. In this situation it
...
\verb|%*l[|{\em fmt.inter.name}\/\verb|]|. The name format specifier 0 is
...
help you having an eye on these two name format specifiers \BibTool{} issues a
...
If {\it sign} is \verb|+| then the words are translated to upper case.  If
{\it sign} is \verb|-| then the words are translated to lower case.  If no
...
The number {\it len}\/ can be used to specify the number of characters to be
used. Each word is truncated to at most {\it len}\/ characters if {\it len}\/
...
If {\tt[}{\it mid}{\tt]} is given then this string is used between several
words of the given part. If none is given then the empty string is used.

If {\tt[}{\it pre}{\tt]} is given then this string is used before the given
part, but only if the part is not empty. If none is given then the empty
string is used.

If {\tt[}{\it post}{\tt]} is given then this string is used after the given
part, but only if the part is not empty. If none is given then the empty
string is used.
...
\begin{Summary}
  ...
  \Desc{}{\rsc{tex.define}\{macro=text\}}{Expand the \TeX{} macro {\it
      macro}\/to {\it text}.}
...
not rewritten (see next section) even so rewrite rules are applicable.
...
This instruction replaces the contents of the field {\it field}\/ by {\it
  value}\/ in each entry. If this field does not exist already then it is
added first. The additions are applied in the sequence they are given.

{\it value}\/ can contain formatting instructions already introduced in the
...
following instruction can be used. It makes use the pseudo fields (see
...
The following instruction deletes all fields named {\it field}:
...
appearance of a \BibTeX{} data base. The powerful facility of regular
expression matching is used for this purpose as we have already seen in
section~\ref{sample.norm}. 
...
can easily be achieved by additionally turn on sorting when requesting
checking of doubles.
...
does not deserve a special treatment. You can simply give the macro file as
one of the input files---preferably before any input file that makes use of
one of the macros contained therein.
...
The distribution of \BibTool{} also contains a file named {\sf ToDo}. If you
