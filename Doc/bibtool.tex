%%*****************************************************************************
%% $Id: bibtool.tex,v 1.20 2011-11-13 13:52:38 gene Exp $
%%=============================================================================
%%
...
hat. Thus all characters but the hash sign (\verb|#|\index{\#}) are allowed.
...
This can be achieved with a resource file containing the following\index{l@\%l}
...
\BibTool\ is hosted in a public repoository at Sarovar. The repository
...
Instead of excessively using command line arguments it is preferrable and less
error prome to put the major configuration into a resource file and just
...
whitespace characters and\index{\#}\index{"}\index{\%}\index{=}\index{,}%
...
\RSC{sort.format}{\%N(author)}\vspace{-4ex}\index{N@\%N}
\RSC{sort.format}{\%N(editor)}

Those two lines are equivalent with the single resource

\RSC{sort.format}{\textit{\%N(author) \# \%N(editor)}}\index{N@\%N}
...
resource \rsc{select.crossrefs}. This resource is |off| by default.
...
\begin{itemize}
  \item [\texttt{p}] \index{p@\%p|(}Format names according to the format specifier number
  ...
  \item [\texttt{s}] \index{s@\%s|(}Take a field as is (after translation of special
        characters).\\ 
  ...
  \item [\texttt{\#w}] \index{w@\%\#w|(}Count the number of words. All words are considered
...
word. For example, with the format \verb|%t(title)|\index{t@\%t}, and 
this field:
\begin{itemize}
...
combined to form a disjunction.  Thus the following code---the constant in
this example---is reached only if we are in a book or in a proceedings entry.
It is not reached in an article.
...
ignored.  Any unwanted spaces after the equality sign may have strange
effects.
...
\RSc{rewrite.rule}{\(\{\) time ".*" = "\%3s(\$mon) \%s(\$day), \%2d(\$year)"\(\}\)}\index{s@\%s}\index{d@\%d}
